...
class Dialog {
    ...
    hide() {
        ...
        document.title = 'OWL DROP | Transfer Files Cross-Platform. No Setup, No Signup.';
...
class ReceiveFileDialog extends ReceiveDialog {
    ...
    async _displayFiles(peerId, displayName, connectionHash, files, imagesOnly, totalSize, badgeClassName) {
        ...
        document.title = files.length === 1
            ? `${ Localization.getTranslation("document-titles.file-received") } - OWL DROP`
            : `${ Localization.getTranslation("document-titles.file-received-plural", null, {count: files.length}) } - OWL DROP`;
...
class ReceiveRequestDialog extends ReceiveDialog {
    ...
    _showRequestDialog(request, peerId) {
        ...
        document.title =  `${transferRequestTitle} - OWL DROP`;
        changeFavicon("images/favicon-96x96-notification.png");
...
class ReceiveTextDialog extends Dialog {
    ...
    _setDocumentTitleMessages() {
        document.title = this._receiveTextQueue.length <= 1
            ? `${ Localization.getTranslation("document-titles.message-received") } - OWL DROP`
            : `${ Localization.getTranslation("document-titles.message-received-plural", null, {count: this._receiveTextQueue.length + 1}) } - OWL DROP`;
    }
